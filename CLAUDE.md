...
**Chunk execution cadence.** 30 Hz sub-step dispatch → one chunk = `F × S` sub-steps (≈1.07 s at F=2, ≈2.13 s at F=4). Keyframes are captured every `CAPTURE_EVERY=4` sub-steps and shipped back to the server in a `compute_kv_cache` call so the server's autoregressive context tracks reality, giving `F×S/CAPTURE_EVERY` keyframes for chunks 1+ (**8 at F=2, 16 at F=4**). Chunk 0 skips frame 0 — the model treats it as "observe the start position" — so it executes `(F-1)×S` sub-steps and yields `(F-1)×S/CAPTURE_EVERY` keyframes (**4 at F=2, 12 at F=4**); within each executed frame the captures fire at `f=3,7,11,15`. **Contract: the server's `compute_kv_cache` must expect exactly this keyframe count for the active horizon — too few/many silently desyncs its KV cache from reality with no error (same failure class as the camera color-order contract).**
